...
def plot_ntk_master_dashboard(
    ...
    for name, data in comps:
        eig = data["eigenvalues"]
        ax2.hist(np.log10(eig), bins=30, color=data["color"], alpha=0.4, 
                 histtype='stepfilled', label=name)
        ax2.hist(np.log10(eig), bins=30, color=data["color"], histtype='step', linewidth=1.5)
    ...
    for name, data in comps:
        if name in ["Внутренние (K)", "PDE (K_L)", "Полная (K_tot)"]:
            eig = data["eigenvalues"]
            k_idx = np.arange(1, len(eig) + 1)
            log_k = np.log10(k_idx)
            log_eig = np.log10(eig)
            ax4.plot(log_k, log_eig, marker=data["marker"], color=data["color"], 
                     linestyle='', markersize=3, alpha=0.6, label=f'{name}')

            alpha = data["metrics"]["decay"]["power_law_alpha"]
            c = np.mean(log_eig + alpha * log_k)
            ax4.plot(log_k, -alpha * log_k + c, color=data["color"], linestyle='--', 
                     linewidth=2, label=f'Степ. $\\alpha={alpha:.2f}$')
    ax4.set_title("4. Затухание (Основная система)", fontweight='bold')
    ...
    for name, data in comps:
        if name in ["Дирихле (K_D)", "Нейман (K_N)"]:
            has_boundaries = True
            eig = data["eigenvalues"]
            k_idx = np.arange(1, len(eig) + 1)
            log_k = np.log10(k_idx)
            log_eig = np.log10(eig)
            ax5.plot(log_k, log_eig, marker=data["marker"], color=data["color"], 
                     linestyle='', markersize=3, alpha=0.6, label=f'{name}')

            alpha = data["metrics"]["decay"]["power_law_alpha"]
            c = np.mean(log_eig + alpha * log_k)
            ax5.plot(log_k, -alpha * log_k + c, color=data["color"], linestyle='--', 
                     linewidth=2, label=f'Степ. $\\alpha={alpha:.2f}$')
    if not has_boundaries:
        ax5.text(0.5, 0.5, "Нет граничных точек", ha='center', va='center', color='gray')
    ax5.set_title("5. Затухание (Граничные условия)", fontweight='bold')
    ...
    ax8.set_ylabel("Ранг")
    ax8.tick_params(axis='x', rotation=12, labelsize=9)
    ...
    ax9.tick_params(axis='x', rotation=12, labelsize=9)
    for bar, val in zip(bars3, frob_vals):
        ax9.text(bar.get_x() + bar.get_width()/2, bar.get_height() + (max(frob_vals)*0.02), 
                 f"{val:.1e}", ha='center', va='bottom', fontsize=9)
    ax9.grid(axis='y', alpha=0.3)

    fig.suptitle(f"Панель спектрального анализа NTK | Эпоха {epoch}", fontsize=18, fontweight='bold', y=0.98)
