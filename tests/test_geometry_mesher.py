...
class TestGetInsideMask(unittest.TestCase):
    ...
    def test_hollow_square(self):
        d = make_domain("hollow_square"); bv = d.boundary_vertices(); bs = d.boundary_segments()
...
class TestMesher(unittest.TestCase):
    def setUp(self): self.mesher = Mesher(max_area=0.1, lloyd_iters=1, boundary_density=40)
    def _validate_mesh(self, mesh, domain):
    ...
    def test_square_mesh(self):
        m = self.mesher.build(make_domain("square")); self._validate_mesh(m, None)
        self.assertGreater(len(m["triangles"]), 10)
    def test_l_shape_mesh(self): self._validate_mesh(self.mesher.build(make_domain("l_shape")), None)
    def test_hollow_square_mesh(self): self._validate_mesh(self.mesher.build(make_domain("hollow_square")), None)
    def test_circle_mesh(self): self._validate_mesh(self.mesher.build(make_domain("circle")), None)
    def test_p_shape_mesh(self): self._validate_mesh(self.mesher.build(make_domain("p_shape")), None)
