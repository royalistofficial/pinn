...
class _SolutionTestMixin:
    solution = None
    def test_eval_shape(self): self.assertEqual(self.solution.eval(torch.randn(10,2)).shape, (10,1))
    def test_grad_shape(self):
        ux, uy = self.solution.grad(torch.randn(10,2))
        self.assertEqual(ux.shape, (10,1)); self.assertEqual(uy.shape, (10,1))
    def test_rhs_shape(self): self.assertEqual(self.solution.rhs(torch.randn(10,2)).shape, (10,1))
    def test_grad_vector_shape(self): self.assertEqual(self.solution.grad_vector(torch.randn(10,2)).shape, (10,2))
    def test_grad_matches_autograd(self):
        xy = torch.randn(20,2, requires_grad=True); u = self.solution.eval(xy)
    ...
    def test_neumann_data(self):
        xy = torch.randn(10,2); normals = torch.randn(10,2)
...
class TestSineSolution(_SolutionTestMixin, unittest.TestCase):
    ...
    def test_zero_on_integer_grid(self):
        ...
        torch.testing.assert_close(self.solution.eval(xy), torch.zeros_like(self.solution.eval(xy)), atol=1e-6, rtol=0)

class TestExponentialSolution(_SolutionTestMixin, unittest.TestCase):
    solution = ExponentialSolution()
    def test_positive(self): self.assertTrue(torch.all(self.solution.eval(torch.randn(20,2)) > 0))

class TestPolynomialSolution(_SolutionTestMixin, unittest.TestCase):
    solution = PolynomialSolution()
    def test_zero_at_origin(self):
        self.assertAlmostEqual(self.solution.eval(torch.tensor([[0.0,0.0]])).item(), 0.0, places=10)
